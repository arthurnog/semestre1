...
while linha != "q":
    ...
    if dados[0] == ">":
        try:
            ...
            print("%s - %s - %s - %s" % (aluno.ra, aluno.email, aluno.nome, aluno.usuario))
        except RAInvalido:
            print("Aluno %s não encontrado" % dados[1])
    elif dados[0] == "+":
        try:
            aluno = Aluno(' '.join(dados[5:]), dados[1], dados[2], dados[3], dados[4])
            repositorio.adicionar(aluno)
            print("Adicionado: %s - %s - %s - %s" % (aluno.ra, aluno.email, aluno.nome, aluno.usuario))
        except RAInvalido:
            print("Aluno %s não encontrado" % dados[1])
        except SenhaFraca:
        ...
        try:
            ...
            print("Alterado: %s - %s - %s - %s" % (aluno.ra, aluno.email, aluno.nome, aluno.usuario))
        except RAInvalido:
            print("Aluno %s não encontrado" % dados[1])
        except SenhaFraca:
